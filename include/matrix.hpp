...
class Matrix {
  public:
    ...
    /// @brief Performs matrix multiplication between 2 matrices with sizes n * m, m * p
    /// @param obj Other matrix
    /// @return Returns matrix with size n * p
    Matrix operator*(Matrix &obj) const {
        if (y != obj.x) {
            throw std::invalid_argument("Shape mismatch, inner shapes must be the same");
        }
        ...
        for (int i = 0; i < x; i++) {
            for (int j = 0; j < obj.y; j++) {
                temp.data[i][j] = dotProduct(data[i], getColumn(obj.data, j)); // getColumn might be inefficient
            }
        }
